...
export function ClientsList({ clients, onClientAdded }: Props) {
  ...
  // حالات التسديد (استخدام string لمنع مشاكل NaN أثناء الكتابة)
  ...
  const handleOpenPayModal = (client: Client) => {
    ...
    setPayAmount(client.outstanding_balance ? client.outstanding_balance.toString() : '');
    setError(null);
  };
  ...
  async function handlePaymentSubmit(e: React.FormEvent) {
    ...
    try {
      // 1. جلب أحدث بيانات للعميل لضمان دقة العمليات الحسابية
      ...
      const newBalance = Math.max(0, currentBalance - numericPayAmount);
      const newTotalPaid = currentTotalPaid + numericPayAmount;
              ...
              <div className="bg-emerald-50/50 p-3 rounded-xl border border-emerald-100 text-xs space-y-1.5">
                <div className="flex justify-between text-slate-600">
                  ...
                  <span className="font-bold font-mono text-slate-900">
                    {formatILS(Math.max(0, (showPayModal.outstanding_balance || 0) - numericPayAmount))}
                  </span>
                </div>
                <div className="flex justify-between text-slate-600">
                  <span>إجمالي المدفوعات الجديد:</span>
                  <span className="font-bold font-mono text-emerald-600">
                    {formatILS((showPayModal.total_paid || 0) + numericPayAmount)}
                  </span>
                </div>
              </div>
