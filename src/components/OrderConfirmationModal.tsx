...
export function OrderConfirmationModal({ summary, onClose, onPrint }: Props) {
      ...
      try {
        for (const item of summary!.items) {
          if (isLensItem(item)) {
            ...
            if (item.cyl != null) query = query.eq('cyl', item.cyl);
            if (item.axis != null) query = query.eq('axis', item.axis);
  ...
  return (
    <>
      {/* ==================== 1. واجهة النافذة المنبثقة (تختفي عند الطباعة) ==================== */}
      <div
        className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in print:hidden"
        onClick={onClose}
        dir="rtl"
      >
        <div
                ...
                {summary.items.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between text-sm border-b border-slate-100 pb-2">
                    ...
                    <div className="text-left flex-shrink-0">
                      ...
                      {isLensItem(item) && <div className="text-xs text-slate-400">×{item.quantity}</div>}
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* ==================== 2. قالب الطباعة المدمج (يظهر فقط أثناء الطباعة) ==================== */}
      <div className="hidden print:block print:w-full print:p-6 text-slate-900 font-sans" dir="rtl">
        {/* الترويسة ورقم الفاتورة */}
        <div className="flex justify-between items-start pb-4 mb-4 border-b-2 border-sky-600">
          <div>
            <h1 className="text-2xl font-black text-sky-600">شركة ومستودع الرؤيا النقية</h1>
            <p className="text-xs text-slate-500 mt-1">
              التاريخ: {new Date(summary.createdAt).toLocaleDateString('ar-EG')}
            </p>
          </div>
          <div className="text-left">
            <h2 className="text-xl font-bold text-slate-800">فاتورة: {summary.invoiceNumber}</h2>
          </div>
        </div>

        {/* قسم اسم العميل (مضمّن هنا بوضوح) */}
        <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 mb-6 flex justify-between items-center">
          <div>
            <span className="text-xs text-slate-500 block">اسم العميل:</span>
            <span className="text-base font-bold text-slate-900">{summary.client?.name || 'عميل نقدي'}</span>
          </div>
          {summary.client?.code && (
            <div className="text-left">
              <span className="text-xs text-slate-500 block">كود العميل:</span>
              <span className="text-sm font-semibold text-slate-700">{summary.client.code}</span>
            </div>
          )}
        </div>
        ...
        <table className="w-full text-right border-collapse mb-6">
          <thead>
            <tr className="bg-slate-100 text-slate-700 text-sm font-bold border-b border-slate-300">
          ...
          <tbody className="divide-y divide-slate-200 text-sm">
            {summary.items.map((item, idx) => (
              <tr key={idx}>
                ...
                <td className="py-2 px-3 text-center font-mono">{formatILS(item.unitPrice)}</td>
                <td className="py-2 px-3 text-left font-mono font-semibold">
                  {formatILS(item.unitPrice * item.quantity)}
                </td>
              </tr>
            ))}
        ...
        {/* طريقة الدفع والمجاميع */}
        <div className="flex justify-between items-start pt-2 border-t-2 border-sky-600">
          <div className="text-sm font-medium pt-2">
            <span>طريقة الدفع: </span>
            <span className="font-bold">{PAYMENT_LABELS[summary.paymentMethod] || 'نقدي'}</span>
          </div>
          ...
          <div className="w-64 space-y-1.5 text-left">
            {summary.discountAmount > 0 && (
              <div className="flex justify-between text-sm text-slate-600">
                <span>المجموع:</span>
                <span className="font-mono">{formatILS(summary.subtotal)}</span>
              </div>
            )}
            {summary.discountAmount > 0 && (
              <div className="flex justify-between text-sm text-rose-600">
                <span>الخصم:</span>
                <span className="font-mono">- {formatILS(summary.discountAmount)}</span>
              </div>
            )}
            <div className="flex justify-between text-lg font-black text-sky-600 border-t pt-1">
              <span>الإجمالي:</span>
              <span className="font-mono">{formatILS(summary.total)}</span>
            </div>
          </div>
        </div>
      </div>
