...
export function OrdersHistory({ onPrintInvoice }: OrdersHistoryProps) {
  ...
  // جلب البيانات من الخادم مع معالجة آمنة تضمن إظهار الجدول دائماً
  async function fetchOrders() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`تعذر الاتصال بمصدر البيانات (${res.status})`);
      }
      const data = await res.json();
      setOrders(Array.isArray(data) ? data : []);
    } catch (err: any) {
      console.warn('تنبيه جلب الطلبات:', err?.message || err);
      // في حال عدم وجود المسار أو حدوث خطأ، نعيد مصفوفة فارغة لعرض الجدول بشكل طبيعي بدلاً من إيقاف الشاشة
      setOrders([]);
    } finally {
      setLoading(false);
    }
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-300 space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <button
              onClick={fetchOrders}
              disabled={loading}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-sm text-xs font-semibold text-slate-800 bg-white border border-slate-300 hover:bg-slate-100 transition-colors disabled:opacity-50"
            >
              <svg className={`w-3.5 h-3.5 text-slate-600 ${loading ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <table className="w-full text-right border-collapse">
            ...
            <tbody className="divide-y divide-slate-200 text-xs text-slate-800">
              {filteredOrders.length > 0 ? (
                ...
                <tr>
                  <td colSpan={8} className="py-10 text-center text-slate-500 font-medium text-xs">
